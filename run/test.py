# File: main.py
# (Chỉ cần đảm bảo tên thư mục output đúng)

import sys
import os
import glob
import shutil # Giữ lại để dọn dẹp
...
OUTPUT_FOLDER_CROPPED = 'data/cropped_lines_processed' # Thư mục chứa ảnh đã cắt, xử lý, resize
...
PADDING_RATIO = 0.17
TARGET_OCR_HEIGHT = 64 # Chiều cao chuẩn cho VietOCR
KERNEL_SIZE = 2 # Kích thước kernel cho phép Đóng
# ----------------------------------

# --- CHỌN HÀNH ĐỘNG ---
RUN_DETECTION = True
DRAW_DEBUG_IMAGE = True # Có thể tắt nếu muốn
RUN_CROPPING = True
RUN_RECOGNITION = True # Bật nhận dạng
RUN_ENHANCE_LETTER = True
# ---------------------------------
...
if __name__ == "__main__":
    ...
    # Dọn dẹp thư mục cũ
    clean_folder(OUTPUT_FOLDER_CROPPED)
